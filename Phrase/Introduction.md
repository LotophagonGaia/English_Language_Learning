...
短语本身可以由一个单词或一组单词构成。我们可以用**替换（substitution）**的方式来检验某个单位是否为短语：如果一组单词可以被一个单词（即单单词短语）替换而不改变基本含义，那么这组单词就可以被认为是一个短语。如上文的例句1可以进行如下替换：*The opposition* 可被 *It* 替换，*a more representative government* 可被 *something* 替换。替换后，该句的含义可保持不变。

> [It] [demands] [something].

我们也可以通过**移位测试（movement tests）**来识别短语：它可以作为一个整体位移到不同位置。可见下文的例句1a：在位移后，句子含义可保持不变。
...
>
> B: Yeah, it's [twenty minutes to four].（嗯，差二十分钟四点。）
>
...
我们可以看到，名词短语和介词短语在书面语中可以相当复杂，有时会呈现出多层嵌套结构。短语的复杂程度是衡量不同语域中英语句法复杂性的一个重要指标（这里的"语域"指语言在不同使用场合、交际目的和交际对象下形成的不同风格或变体）。一般而言，短语结构在日常对话中相对简单，在小说和新闻写作中逐渐复杂，在学术写作中通常达到较高水平。
